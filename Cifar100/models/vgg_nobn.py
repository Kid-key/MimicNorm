...
def vgg11_nobn():
    return VGG(make_layers(cfg['A']),class_bn=False)

def vgg13_cbn():
    return VGG(make_layers(cfg['B']),class_bn=True)
    # return VGG(make_layers(cfg['B'], batch_norm=True))
def vgg13_nobn():
    return VGG(make_layers(cfg['B'],batch_norm=True),class_bn=False)

def vgg16_cbn():
    return VGG(make_layers(cfg['D']),class_bn=True)
    # return VGG(make_layers(cfg['D'], batch_norm=True))
def vgg16_nobn():
    return VGG(make_layers(cfg['D']),class_bn=False)

def vgg19_cbn():
    return VGG(make_layers(cfg['E']),class_bn=True)
    # return VGG(make_layers(cfg['E'], batch_norm=True))
def vgg19_nobn():
    return VGG(make_layers(cfg['E'],batch_norm=True),class_bn=False)
